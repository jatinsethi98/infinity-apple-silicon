...
if __name__ == '__main__':
    ...
    parser.add_argument('-f', '--file', help='Input file path (one text per line)')
    ...
    if args.file:
        ...
        if not os.path.exists(args.file):
            logging.error(f"Input file does not exist: {args.file}")
            exit(1)
        ...
        logging.info(f"Read {len(texts)} texts from file")

    elif args.text:
        # Single text mode
        texts = [args.text]
        logging.info(f"Processing single text: {args.text}")

    else:
        # Use default test cases
        logging.info("No input text provided, using built-in test cases")
        texts = [
    ...
    for i, text in enumerate(texts, 1):
        try:
            if args.fine_grained:
                ...
                basic_result = tokenizer.tokenize(text)
                result = tokenizer.fine_grained_tokenize(basic_result)
            else:
                # Basic tokenization
