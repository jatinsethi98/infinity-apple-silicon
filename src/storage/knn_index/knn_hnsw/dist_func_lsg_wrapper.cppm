...
public:
    ...
    LSGDistWrapper(LSGDistWrapper &&other) noexcept
        : avg_(std::exchange(other.avg_, nullptr)), alpha_(other.alpha_), dist_(std::move(other.dist_)) {
        static_assert(std::is_nothrow_move_constructible_v<Dist>);
    }
    LSGDistWrapper &operator=(LSGDistWrapper &&other) noexcept {
        static_assert(std::is_nothrow_move_assignable_v<Dist>);
        if (this != &other) {
